...
def try_models(trainX, trainY, ids, models, N=5, scoring=None, scale=False, save_trail='', save=False):
    ...
    return all_runs, statistics
    # np.savetxt(config['model_dir']+'/runs_'+save_trail+'.csv', all_runs, fmt='%s', delimiter=',', header='Model,Score,Runtime', comments='')
    # np.savetxt(config['model_dir']+'/statistics_'+save_trail+'.csv', statistics, fmt='%s', delimiter=',', header='Model,Mean_score,Std_score,Mean_runtime,Std_runtime', comments='')

def benchmark(trainX, trainY):
    config_arr = [config['task'], config['dataset'], config['preprocessing']]
    np.savetxt(config['model_dir']+'/config.csv', config_arr , fmt='%s')
    ...
    if config['task'] == 'LR_task':
        if config['dataset'] == 'antisaccade':
            ...
            all_runs, statistics = try_models(trainX=trainX, trainY=y, ids=ids, models=models, save_trail=str(trainX.shape[1]), scoring=scoring)
        else:
        ...
        if config['dataset'] == 'dots':
            ...
            all_runs, statistics = try_models(trainX=trainX, trainY=y2, ids=ids, models=models['angle'], scoring=scoring2, save_trail='_angle')
        else:
            raise ValueError("This task cannot be predicted (is not implemented yet) with the given dataset.")
        ...
        if config['dataset'] == 'dots':
            ...
            all_runs, statistics = try_models(trainX=trainX, trainY=y, ids=ids, models=models, scoring=scoring2)
        else:
            raise ValueError("This task cannot be predicted (is not implemented yet) with the given dataset.")
    else:
