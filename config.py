...
import time
import os
from feature_selection.config_FS import config_FS
config = dict()
...
config['log_dir'] = config_FS['log_dir']
# Path to raw data before split.
...
config['first_half'] = config_FS['first_half']
config['full_data'] = config_FS['full_data']
...
def create_folder():
    if config['retrain']:
        ...
        if config['full_data']:
            model_folder_name += '_full'
        else:
            if config['first_half']:
                model_folder_name += '_first'
            else:
                model_folder_name += '_second'
